 ...
 ##   初始化 
  ...
  ##      模板大小3*3 
       ...
       ## 橫向直線列表 
        # horizontal_lines = [] 
        # for i in range(h - 1): 
        ##    找到兩條記錄的分隔線段，以相鄰兩行的平均像素差大於120為標準 
  ...
  ##  縱向直線檢測 
   ...
   ## 尋找單元格區域 
    # def CellDetect(self): 
        ...
        # horizontal_lines = self.HorizontalLineDetect() 
       ## 頂點列表 
        # rects = [] for i in range(0, len(vertical_lines) - 1, 2): 
    ...
    # def OCR(self): 
       ...
       ## 特殊字符列表 
        ...
        # for i in range(20): 
            ...
            ##識別數字（每行第一列） 
            # text1 = pytesseract.image_to_string(DetectImage1, config="--psm 10", lang='chi_tra') 
            # print(text1, end='-->') 
# 
            ##識別漢字（每行第二列） 
            # rect2 = rects[i+20] 
            # DetectImage2 = thresh[rect2[1]:rect2[3], rect2[0]:rect2[2]] 
            # text2 = pytesseract.image_to_string(DetectImage2, config='--psm 7', lang='chi_tra') 
...
# 
            ...
            # text3 = pytesseract.image_to_string(DetectImage3, config='--psm 7', lang='chi_tra') 
...
ImagePath = 'PDFtoEXCEL_ch0_1.png' 
imageOCR = ImageTableOCR(ImagePath) 
# print(ImagePath)
# print(type(imageOCR.OCR()))
imageOCR.OCR()
# ImageTableOCR.OCR(ImagePath)

# 原文網址：https://kknews.cc/code/kvag33b.html
